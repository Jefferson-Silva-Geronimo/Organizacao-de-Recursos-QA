...
> **Data:** 2026-09-11  
> **Status:** Gate TDD final em 2026-09-25 (ver seção 11). Fase GREEN dos 22 casos: seção 10. Ciclo RED dos casos T-RFnn: seção 9. Fase GREEN anterior: seção 8. Fase RED: seção 7. Estado anterior (2026-09-11): Preparação para ciclo de TDD  
...
| RF-20 | **T-RF20-001** | RF-20, critério de aceitação 1 (docs/prd.md:7.20) | Então o sistema produz a notificação simulada ou realiza a chamada externa definida. | `NotificacaoReserva_RF20_Test` | T-RF20-001: Happy Path - Evento concluído produz a notificação simulada | unitário (ServicoNotificacao com canal de notificação simulada) | **GREEN** | Parte verificável coberta: um evento concluído produz a notificação pelo canal simulado e o resultado fica registrado. Antes: BLOQUEADO_POR_LACUNA (ciclo RED 2026-09-25). RED inicial: 2 execuções falharam por asserção (canal sem notificações, registros vazios). | mvn -B -o verify 2026-09-25: aprovado (ver seção 11) | Continuam abertos: quais eventos do fluxo notificam e a ligação com as operações de reserva, canal real (WireMock ou API externa), destinatário e conteúdo (arquitetura J15; fluxos-personas, notificações). RF-20 tem cobertura automatizada PARCIAL. |
| RF-20 | **T-RF20-002** | RF-20, critério de aceitação 2 (docs/prd.md:7.20) | Então o sistema registra o resultado da falha de forma segura e observável. | `NotificacaoReserva_RF20_Test` | T-RF20-002: Invalid Input - Canal que não responde tem a falha registrada sem expor detalhes internos | unitário (ServicoNotificacao com canal que falha) | **GREEN** | Parte verificável coberta: falha do canal é registrada como resultado observável, não propaga e não expõe detalhes internos. Antes: BLOQUEADO_POR_LACUNA. RED inicial: registros vazios e mensagem em branco. | mvn -B -o verify 2026-09-25: aprovado (ver seção 11) | Continuam abertos: teste com WireMock e integração real (RNF-08), política de retry (não definida para notificação), evento, destinatário e conteúdo. |
...
| RF-22 | **T-RF22-002** | RF-22, critério de aceitação 2 (docs/prd.md:7.22) | Então o sistema apresenta mensagem compreensível e não expõe informação sensível. | `ErrosCompreensiveis_RF22_Test` | T-RF22-002: Recusas conhecidas informam a causa observável sem expor informação sensível | unitário e integração de componentes (TradutorErros com recusas reais do domínio) | **GREEN** | Reclassificado de SEM_COMPORTAMENTO_VERIFICAVEL: a parte objetiva é verificável (causa observável nomeada nas recusas de período inválido, conflito, manutenção e falta de autorização; falha interna e causa encadeada sem stack trace, credenciais ou detalhes internos, conforme fluxos-personas FLX-01, RNF-15 e RNF-17). RED inicial: 5 execuções falharam com mensagem vazia. | mvn -B -o verify 2026-09-25: aprovado (ver seção 11) | Continuam abertos: o caráter compreensível para o usuário final é revisão de QA (RNF-17); interface, viewports (T-RF22-001 bloqueado) e camada de apresentação. RF-22 tem cobertura automatizada PARCIAL (somente critério 2). |
| RF-23 | **T-RF23-001** | RF-23, critério de aceitação 1 (docs/prd.md:7.23) | Então ela descreve os fluxos ou contratos disponibilizados pelo sistema. | `DocumentacaoFluxos_RF23_Test` | T-RF23-001: Happy Path - Estados e transições confirmados na documentação correspondem ao comportamento | documental semântico (matriz de estados de docs/fluxos-personas.md x código) | **REGRA_JA_IMPLEMENTADA** | Reclassificado de BLOQUEADO_POR_LACUNA: a documentação de fluxos já existe (docs/fluxos-personas.md, seção 12) e o teste confronta seu conteúdo com ValidadorFluxoEstados e ValidadorApagamento (RNF-18: comparação com a baseline). Passou de imediato; investigado por mutação (remover a transição SOLICITADA para REJEITADA no código faz o teste falhar). | mvn -B -o verify 2026-09-25: aprovado (ver seção 11) | Continuam abertos: escolha entre documentar API ou fluxos públicos, formato, escopo e publicação (FLX-22, pendências; RNF-18). Somente as linhas confirmadas da matriz são confrontadas; as marcadas PENDENTE DE DECISÃO não. |
| RF-23 | **T-RF23-002** | RF-23, critério de aceitação 2 (docs/prd.md:7.23) | Então essas condições observáveis estão incluídas na documentação. | `DocumentacaoFluxos_RF23_Test` | T-RF23-002: Happy Path - Autorizações documentadas por perfil correspondem ao comportamento | documental semântico (matriz de permissões de docs/fluxos-personas.md x autorização real) | **REGRA_JA_IMPLEMENTADA** | Reclassificado de BLOQUEADO_POR_LACUNA: confronta cada célula PERMITIDO ou NEGADO da matriz de permissões (seção 11) com a autorização real de 11 operações. Passou de imediato; investigado por mutação (remover a permissão VALIDAR_ALOCACAO_DOCENTE do Responsável faz o teste falhar). | mvn -B -o verify 2026-09-25: aprovado (ver seção 11) | Continuam abertos: células CONDICIONAL e PENDENTE DE DECISÃO não são confrontadas; erros documentados por fluxo não têm teste próprio; escolha do artefato oficial de documentação (FLX-22). |
...
## 10. Fase GREEN dos 22 casos T-RFnn

> As seções 7, 8 e 9 foram preservadas. Esta seção converte em GREEN os 22 casos que a seção 9.3 classificou como RED_FALHA_ESPERADA (lista extraída da matriz atual e reproduzida por execução antes de qualquer alteração: 22 casos, 31 execuções vermelhas mais T-RN10-003).
> **Execução final:** `mvn -B -o verify` em 2026-09-25 · 154 execuções de teste · 153 aprovadas · **1 falha (T-RN10-003)** · 0 erros · 16 ignorados (os mesmos bloqueios anteriores, intactos).
> **JaCoCo (informativo; gerado com `-Dmaven.test.failure.ignore=true`):** 80,1% de linhas (meta 80%: atendida) e 61,1% de branches (meta 70%: **não atendida**). Nenhum teste foi criado para elevar o percentual.
> **Regressões:** nenhuma; os 123 testes anteriores seguem aprovados, exceto T-RN10-003, que já estava vermelho.

### 10.1 Matriz dos 22 casos

| RF | Caso | Classe e método de teste | Status anterior | Status atual | Comportamento implementado | Fonte utilizada | Classe de produção alterada | Evidência da execução | Regressões | Observações ou divergências |
|---|---|---|---|---|---|---|---|---|---|---|
| RF-02 | **T-RF02-001** | `GestaoSalas_RF02_Test` - T-RF02-001: Happy Path - Administrador cadastra uma sala e ela pode ser consultada | RED_FALHA_ESPERADA | **GREEN** | cadastrarRecurso registra a sala (somente Administrador) e consultarRecursos(SALA) a devolve | PRD RF-02 critério 1; ADR-003 | CadastroRecursos | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma | Cadastro em memória; persistência real não faz parte do caso. |
| RF-02 | **T-RF02-002** | `GestaoSalas_RF02_Test` - T-RF02-002: Forbidden State - Solicitante sem permissão de gestão não cadastra sala | RED_FALHA_ESPERADA | **GREEN** | Cadastro de sala por Solicitante é recusado com AcessoNegadoException (permissão GERENCIAR_RECURSOS) e a sala não é registrada | PRD RF-02 critério 2; ADR-003 | CadastroRecursos | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma |  |
| RF-03 | **T-RF03-001** | `GestaoProfessores_RF03_Test` - T-RF03-001: Happy Path - Administrador cadastra um professor e ele pode ser consultado | RED_FALHA_ESPERADA | **GREEN** | cadastrarProfessor registra o professor (somente Administrador) e consultarProfessores o devolve | PRD RF-03 critério 1; ADR-003 | CadastroRecursos | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma | A relação professor-usuário/recurso (Q-010) não foi tocada. |
| RF-03 | **T-RF03-002** | `GestaoProfessores_RF03_Test` - T-RF03-002: Forbidden State - Solicitante sem permissão de gestão não cadastra professor | RED_FALHA_ESPERADA | **GREEN** | Cadastro de professor por Solicitante é recusado (AcessoNegadoException) | PRD RF-03 critério 2; ADR-003 | CadastroRecursos | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma |  |
| RF-04 | **T-RF04-001** | `GestaoMateriais_RF04_Test` - T-RF04-001: Happy Path - Administrador cadastra um material e ele pode ser consultado | RED_FALHA_ESPERADA | **GREEN** | cadastrarRecurso registra o material (somente Administrador) e consultarRecursos(MATERIAL) o devolve | PRD RF-04 critério 1; ADR-003 | CadastroRecursos | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma |  |
| RF-04 | **T-RF04-002** | `GestaoMateriais_RF04_Test` - T-RF04-002: Forbidden State - Solicitante sem permissão de gestão não cadastra material | RED_FALHA_ESPERADA | **GREEN** | Cadastro de material por Solicitante é recusado (AcessoNegadoException) | PRD RF-04 critério 2; ADR-003 | CadastroRecursos | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma |  |
| RF-05 | **T-RF05-001** | `GestaoUsuarios_RF05_Test` - T-RF05-001: Happy Path - Administrador gerencia usuário com perfil oficial | RED_FALHA_ESPERADA | **GREEN** | definirPerfil (somente Administrador) devolve o usuário com o novo perfil oficial; a autorização passa a usar as permissões do novo perfil (2 execuções) | PRD RF-05 critério 1; ADR-003 | GestaoUsuarios | mvn -B -o verify 2026-09-25: classe aprovada (4 execuções) | nenhuma | O usuário original não é modificado; não há repositório de usuários. |
| RF-05 | **T-RF05-002** | `GestaoUsuarios_RF05_Test` - T-RF05-002: Forbidden State - Usuário que não é Administrador não gerencia usuários | RED_FALHA_ESPERADA | **GREEN** | Usuário que não é Administrador (Solicitante e Responsável) é recusado ao gerenciar usuários e o perfil do alvo não muda (2 execuções) | PRD RF-05 critério 2; ADR-003 | GestaoUsuarios | mvn -B -o verify 2026-09-25: classe aprovada (4 execuções) | nenhuma |  |
| RF-06 | **T-RF06-001** | `GestaoBloqueios_RF06_Test` - T-RF06-001: Happy Path - Administrador cria bloqueio e o recurso fica indisponível no período | RED_FALHA_ESPERADA | **GREEN** | registrarBloqueio (somente Administrador) torna o recurso indisponível no período do bloqueio e disponível fora dele | PRD RF-06 critério 1; RN-01 (período com término posterior ao início) | GestaoBloqueios | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma | Bloqueios em memória. Política para reservas já existentes afetadas por bloqueio segue pendente (arquitetura §12, item 9). |
| RF-06 | **T-RF06-002** | `GestaoBloqueios_RF06_Test` - T-RF06-002: Forbidden State - Solicitante não gerencia bloqueios | RED_FALHA_ESPERADA | **GREEN** | Registro de bloqueio por Solicitante é recusado (AcessoNegadoException) e o recurso segue disponível | PRD RF-06 critério 2; ADR-003 | GestaoBloqueios | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma |  |
| RF-08 | **T-RF08-001** | `ConsultaRecursos_RF08_Test` - T-RF08-001: Happy Path - Solicitante consulta salas, professores e materiais cadastrados | RED_FALHA_ESPERADA | **GREEN** | Solicitante autenticado consulta salas, materiais e professores cadastrados (permissão CONSULTAR_DISPONIBILIDADE) | PRD RF-08 critério 1; ADR-003 | CadastroRecursos | mvn -B -o verify 2026-09-25: classe aprovada (1 execuções) | nenhuma |  |
| RF-09 | **T-RF09-002** | `DisponibilidadeRecursos_RF09_Test` - T-RF09-002: Conflicts - Recurso indisponível no período não é apresentado como disponível | RED_FALHA_ESPERADA | **GREEN** | Recurso reservado, bloqueado, em manutenção ou com professor de agenda ocupada não é apresentado como disponível no período; fora dele é disponível (4 execuções) | PRD RF-09 critério 2; ADR-005 (revalidar sala, material, professor, manutenção e bloqueio) | ConsultaDisponibilidade; ServicoCriacaoReserva (possuiConflito); ValidadorSobreposicao (existeConflito) | mvn -B -o verify 2026-09-25: classe aprovada (4 execuções) | nenhuma | A pesquisa por filtros (T-RF09-001) segue bloqueada e não foi implementada. |
| RF-11 | **T-RF11-001** | `AlteracaoReserva_RF11_Test` - T-RF11-001: Happy Path - Solicitante altera a própria reserva para um período livre | RED_FALHA_ESPERADA | **GREEN** | alterarReserva do próprio solicitante, reserva não iniciada e período livre, aplica o novo período | PRD RF-11 critério 1; RN-01, RN-02, RN-03, RN-05; ADR-005 | ServicoGestaoReserva; ServicoCriacaoReserva (alterarPeriodo); Professor | mvn -B -o verify 2026-09-25: classe aprovada (5 execuções) | nenhuma | Alteração revalida término posterior ao início, manutenção, sobreposição de sala e material e agenda do professor na seção crítica local. |
| RF-11 | **T-RF11-002** | `AlteracaoReserva_RF11_Test` - T-RF11-002: Forbidden State - Alteração recusada quando não permitida | RED_FALHA_ESPERADA | **GREEN** | Alteração recusada (sem alterar a reserva) quando a reserva é de outro solicitante, já iniciada, conflita com outra reserva ou usa recurso em manutenção (4 execuções) | PRD RF-11 critério 2; RN-08; ADR-003 e ADR-005 | ServicoGestaoReserva; ServicoCriacaoReserva; ValidadorSobreposicao | mvn -B -o verify 2026-09-25: classe aprovada (5 execuções) | nenhuma | Quando há mais de um impedimento, a mensagem exibida é a do primeiro verificado (dono, período, manutenção, conflito, iniciada). |
| RF-12 | **T-RF12-001** | `CancelamentoReserva_RF12_Test` - T-RF12-001: Happy Path - Solicitante cancela a própria reserva ainda não iniciada | RED_FALHA_ESPERADA | **GREEN** | cancelarReserva do próprio solicitante, reserva não iniciada: estado passa a CANCELADA e a mudança é registrada em auditoria (usuário, ação CANCELAR, estado anterior e novo) | PRD RF-12 critério 1; RN-07, RN-08, RN-09 | ServicoGestaoReserva | mvn -B -o verify 2026-09-25: classe aprovada (3 execuções) | nenhuma | A auditoria é gravada antes da mudança de estado; mecanismo transacional definitivo segue pendente (ADR-004). |
| RF-12 | **T-RF12-002** | `CancelamentoReserva_RF12_Test` - T-RF12-002: Forbidden State - Cancelamento recusado com motivo informado | RED_FALHA_ESPERADA | **GREEN** | Cancelamento recusado com motivo quando a reserva está iniciada (RN-08) ou pertence a outro solicitante (2 execuções) | PRD RF-12 critério 2; RN-08; ADR-003 | ServicoGestaoReserva | mvn -B -o verify 2026-09-25: classe aprovada (3 execuções) | nenhuma |  |
| RF-14 | **T-RF14-001** | `AprovacaoSolicitacoes_RF14_Test` - T-RF14-001: Happy Path - Responsável aprova solicitação especial pendente | RED_FALHA_ESPERADA | **GREEN** | aprovar passa a registrar a auditoria da mudança de estado (usuário, ação APROVAR, estado anterior e novo APROVADA); o estado APROVADA já era aplicado | PRD RF-14 critério 1; RN-09; ADR-004 | ValidadorAprovacao | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma | Falhava só a auditoria. |
| RF-14 | **T-RF14-003** | `AprovacaoSolicitacoes_RF14_Test` - T-RF14-003: Happy Path - Responsável rejeita solicitação especial | RED_FALHA_ESPERADA | **GREEN** | rejeitar passa a registrar a auditoria da mudança de estado (ação REJEITAR, estado novo REJEITADA); o estado REJEITADA já era aplicado | PRD RF-14 critério 3; RN-09; ADR-004 | ValidadorAprovacao | mvn -B -o verify 2026-09-25: classe aprovada (2 execuções) | nenhuma | Falhava só a auditoria. |
| RF-15 | **T-RF15-002** | `ValidacaoAlocacaoDocente_RF15_Test` - T-RF15-002: Conflicts - Alocação em conflito com a agenda do professor informa o conflito e não é validada | RED_FALHA_ESPERADA | **GREEN** | validar (somente Responsável) recusa alocação que conflita com a agenda do professor com ReservaAgendaProfessorException "Professor indisponível" e não a considera validada | PRD RF-15 critério 2; RN-03 | ValidacaoAlocacaoDocente; Professor (temConflitoIgnorando); ValidadorAutorizacao (permissão) | mvn -B -o verify 2026-09-25: classe aprovada (1 execuções) | nenhuma | O resultado registrado da validação positiva (T-RF15-001) segue bloqueado. |
| RF-16 | **T-RF16-002** | `RetiradaMateriais_RF16_Test` - T-RF16-002: Forbidden State - Solicitante sem permissão não registra retirada | RED_FALHA_ESPERADA | **GREEN** | Registro de retirada por Solicitante é recusado (AcessoNegadoException; permissão REGISTRAR_MOVIMENTACAO_MATERIAIS, Responsável) | PRD RF-16 critério 2; ADR-003 | MovimentacaoMateriais; ValidadorAutorizacao (permissão); ReservaMovimentacaoException | mvn -B -o verify 2026-09-25: classe aprovada (1 execuções) | nenhuma | A retirada é guardada só em memória para aplicar a regra da devolução; persistência realista (RNF-07) segue pendente e T-RF16-001 continua bloqueado. |
| RF-17 | **T-RF17-002** | `DevolucaoMateriais_RF17_Test` - T-RF17-002: Invalid Input - Devolução sem retirada correspondente é recusada com mensagem compreensível | RED_FALHA_ESPERADA | **GREEN** | Devolução sem retirada correspondente é recusada com ReservaMovimentacaoException e mensagem clara | PRD RF-17 critério 2 | MovimentacaoMateriais; ReservaMovimentacaoException | mvn -B -o verify 2026-09-25: classe aprovada (1 execuções) | nenhuma | Persistência realista (T-RF17-001) segue bloqueada. |
| RF-21 | **T-RF21-002** | `RelatoriosOperacionais_RF21_Test` - T-RF21-002: Forbidden State - Solicitante sem permissão não consulta relatórios operacionais | RED_FALHA_ESPERADA | **GREEN** | Consulta de relatório por Solicitante é recusada (AcessoNegadoException; permissão CONSULTAR_RELATORIOS, Administrador) | PRD RF-21 critério 2; ADR-003 | ServicoRelatorios; ValidadorAutorizacao (permissão) | mvn -B -o verify 2026-09-25: classe aprovada (1 execuções) | nenhuma | Somente a autorização foi implementada; conteúdo do relatório (T-RF21-001) segue bloqueado. |

### 10.2 T-RN10-003 (segue RED_FALHA_ESPERADA)

A verificação não foi alterada. Ela continua falhando por um motivo documental real: RF-20, RF-22 e RF-23 não têm nenhum teste JUnit real, porque todos os seus casos estão bloqueados (T-RF20-001/002, T-RF22-001, T-RF23-001/002) ou sem comportamento verificável (T-RF22-002). Os demais RFs agora têm testes reais e passam nas outras verificações (casos válidos com teste, sem identificadores órfãos, classe registrada na matriz igual à classe do código). Por isso o `verify` termina com uma falha; não foi criada aprovação artificial nem bloqueio foi levantado.

### 10.3 Casos fora do escopo (inalterados)

- **DUPLICADO (10):** T-RF07-001, T-RF07-002, T-RF08-002, T-RF13-001, T-RF13-002, T-RF14-002, T-RF18-001, T-RF18-002, T-RF19-001, T-RF19-002 continuam vinculados aos testes citados na seção 9.3 (todos existentes).
- **BLOQUEADO_POR_LACUNA (11):** T-RF09-001, T-RF11-003, T-RF15-001, T-RF16-001, T-RF17-001, T-RF20-001, T-RF20-002, T-RF21-001, T-RF22-001, T-RF23-001, T-RF23-002.
- **SEM_COMPORTAMENTO_VERIFICAVEL (1):** T-RF22-002.
- Os 16 `@Disabled` anteriores permanecem. Nenhum bloqueio foi removido.

### 10.4 Decisões e divergências

- **Permissões novas em `ValidadorAutorizacao.obterPermissoes`** (acréscimo compatível; os testes existentes só verificam presença): Administrador ganhou CONSULTAR_RELATORIOS (RF-21, persona Administrador); Responsável ganhou VALIDAR_ALOCACAO_DOCENTE (RF-15) e REGISTRAR_MOVIMENTACAO_MATERIAIS (RF-16 e RF-17). Origem: PRD, campo "Persona de origem" de cada RF. Os nomes das permissões são da implementação.
- **Persistência:** cadastros, bloqueios, retiradas e devoluções ficam em memória. Isso satisfaz somente os casos implementados, que não exigem persistência real; **não** atende RNF-07 e não substitui T-RF16-001 e T-RF17-001 (bloqueados). Nenhuma dependência, banco, Testcontainers ou WireMock foi adicionado.
- **Concorrência (ADR-002):** nenhum dos 22 casos é de concorrência. A seção crítica local de `ServicoCriacaoReserva`, agora também usada por `alterarPeriodo`, segue sendo exclusão local ao processo e não é proteção de persistência; a ADR-002 continua pendente de experimento.
- **Agenda do professor:** `Professor` ganhou `temConflitoIgnorando` e `substituirAgenda` para que alterar ou validar uma reserva não conflite com o próprio período já registrado na agenda.
- **RF-11 critério 3:** não implementado (T-RF11-003 bloqueado).
- **Contratos da fase RED:** nomes, pacotes e assinaturas públicas mantidos; só os corpos foram implementados. Foi criada `ReservaMovimentacaoException`, seguindo o padrão das demais exceções de regra.

### 10.5 Gate final do QA

| Verificação | Resultado |
|---|---|
| Lista dos 22 REDs extraída da matriz atual e reproduzida antes da implementação | Sim (22 casos, 31 execuções vermelhas; todas por asserção sobre comportamento ausente, nenhuma por fixture, compilação ou infraestrutura) |
| Cada GREEN corresponde ao comportamento documentado | Sim (coluna "Fonte utilizada") |
| Nenhum teste apagado, desabilitado ou enfraquecido; resultados esperados intactos | Sim (nenhum arquivo de teste foi alterado nesta fase) |
| Nenhuma decisão ausente inventada | Sim; as escolhas de implementação estão em 10.4 |
| Duplicados não reimplementados; bloqueados fora do escopo | Sim |
| Testes anteriormente verdes continuam verdes | Sim |
| T-RN10-003 continua verificando testes reais | Sim (10.2) |
| Matriz atualizada | Sim (10.1) |
| Arquitetura e ADRs respeitadas | Sim, com a ressalva de ADR-002 e RNF-07 em 10.4 |
| Commit ou push | Nenhum |
| **Conclusão** | Os 22 casos com comportamento definido estão GREEN. O `verify` falha somente por T-RN10-003, que só passa quando RF-20, RF-22 e RF-23 tiverem testes reais. Continuam bloqueados 11 casos, 1 sem comportamento verificável e 16 testes `@Disabled`. |

---

## 11. Gate TDD final: cobertura automatizada de RF-20, RF-22 e RF-23

> As seções 7 a 10 foram preservadas. As linhas de RF-20, RF-22 e RF-23 da matriz da seção 9.3 (fonte da verificação de T-RN10-003) foram atualizadas in loco; o status anterior de cada uma está registrado na coluna Motivo daquela matriz.
> **Execução final:** mvn -B -o verify em 2026-09-25 · **BUILD SUCCESS** · 163 testes · 0 falhas · 0 erros · 16 ignorados (@Disabled; os mesmos bloqueios anteriores, intactos).
> **JaCoCo (verify final):** 81,6% de linhas (meta 80%: atendida) e 61,8% de branches (meta 70%: **não atendida**; o pom.xml não impõe gate). Nenhum teste foi criado para elevar o percentual.

### 11.1 Situação inicial de T-RN10-003

Falhava por duas razões: (1) RF-20, RF-22 e RF-23 sem teste JUnit real; (2) um erro técnico do próprio leitor da matriz: ele lia as linhas de todas as seções a partir da seção 9 e, ao encontrar a matriz da seção 10 (outro layout de colunas), comparava classes com valores como GREEN e acusava divergência em 22 casos. Foi corrigido somente o leitor, que agora lê apenas a seção 9 (até a seção 10). Nenhuma verificação foi removida ou enfraquecida.

### 11.2 Análise dos requisitos (fontes: plano, PRD, arquitetura, fluxos-personas)

| RF | Ator e pré-condições | Critérios de aceitação (PRD) | Parte verificável | Parte indefinida ou dependente de infraestrutura |
|---|---|---|---|---|
| RF-20 (Transversal; componente Notificação; J15 e RNF-08) | Evento do fluxo de reserva definido e mecanismo disponível | 1) evento concluído produz a notificação simulada ou a chamada externa; 2) API externa que não responde: falha registrada de forma segura e observável | Um evento concluído produz a notificação simulada; a falha do canal é registrada sem propagar e sem detalhes internos | Quais eventos notificam, canal real (WireMock), destinatário, conteúdo (arquitetura J15 e fluxos-personas: pendentes) |
| RF-22 (todos os perfis; componente Erros; J10 aprovado na baseline) | Fluxo público em viewport suportado; operação válida ou erro conhecido | 1) controles utilizáveis em viewports; 2) recusa por período inválido, conflito, manutenção ou falta de autorização informa mensagem compreensível sem informação sensível | Critério 2 no que é objetivo: causa observável nomeada; sem stack trace, credenciais ou detalhes internos (fluxos-personas FLX-01, RNF-15, RNF-17) | Viewports, interface responsiva e camada de apresentação; o caráter compreensível é revisão de QA |
| RF-23 (Transversal; FLX-22, RNF-18) | API ou fluxos públicos definidos | 1) documentação descreve fluxos ou contratos; 2) estados, autorização e erros relevantes incluídos | Confronto do conteúdo de docs/fluxos-personas.md (matrizes de estados e de permissões) com o comportamento, como pede RNF-18 (comparação da documentação com a baseline) | Escolha entre API e fluxos públicos, formato, escopo e publicação (FLX-22, pendências) |

### 11.3 Matriz dos casos que passaram a ter teste real

| RF | Caso | Requisito de origem | Comportamento esperado (plano) | Classe de teste | Método ou @DisplayName | Nível | Status RED inicial e falha | Status final | Comportamento implementado | Componente alterado | Evidência | Lacunas abertas |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| RF-20 | **T-RF20-001** | RF-20, critério 1 (docs/prd.md:7.20) | produz a notificação simulada quando o evento é concluído | NotificacaoReserva_RF20_Test | T-RF20-001: Happy Path - Evento concluído produz a notificação simulada | unitário | RED_FALHA_ESPERADA: canal sem notificações e registros vazios (asserção) | **GREEN** | ServicoNotificacao aciona o canal e registra o resultado; NotificacaoSimulada guarda as notificações produzidas | ServicoNotificacao, NotificacaoSimulada (contratos da fase RED com corpo implementado; CanalNotificacao, EventoNotificacao e ResultadoNotificacao sem lógica) | 1 execução aprovada | eventos que notificam, canal real, destinatário, conteúdo |
| RF-20 | **T-RF20-002** | RF-20, critério 2 | registra o resultado da falha de forma segura e observável | NotificacaoReserva_RF20_Test | T-RF20-002: Invalid Input - Canal que não responde tem a falha registrada sem expor detalhes internos | unitário | RED_FALHA_ESPERADA: registros vazios (asserção) | **GREEN** | falha do canal vira resultado de falha registrado, sem propagar e com mensagem sem detalhes | ServicoNotificacao | 1 execução aprovada | WireMock e integração real (RNF-08), evento, destinatário, conteúdo |
| RF-22 | **T-RF22-002** | RF-22, critério 2 (docs/prd.md:7.22) | mensagem compreensível, sem informação sensível | ErrosCompreensiveis_RF22_Test | T-RF22-002 (parametrizado): Recusas conhecidas informam a causa observável sem expor informação sensível; e Falha interna e causa encadeada não expõem informação sensível | unitário e integração de componentes | RED_FALHA_ESPERADA: mensagem vazia nas 5 execuções (asserção) | **GREEN** | TradutorErros mantém a mensagem das recusas de regra e devolve mensagem genérica para qualquer outra falha, sem causa encadeada | TradutorErros | 5 execuções aprovadas | interface, viewports (T-RF22-001 bloqueado), revisão de QA do caráter compreensível |
| RF-23 | **T-RF23-001** | RF-23, critério 1 (docs/prd.md:7.23) | documentação descreve os fluxos disponibilizados | DocumentacaoFluxos_RF23_Test | T-RF23-001: Happy Path - Estados e transições confirmados na documentação correspondem ao comportamento | documental semântico | passou de imediato; mutação no código faz o teste falhar | **REGRA_JA_IMPLEMENTADA** | nenhum (documentação já existia); o teste passa a verificar seu conteúdo | nenhum | 1 execução aprovada | escolha API x fluxos, formato e publicação; linhas PENDENTE DE DECISÃO não confrontadas |
| RF-23 | **T-RF23-002** | RF-23, critério 2 | estados, autorização e erros relevantes documentados | DocumentacaoFluxos_RF23_Test | T-RF23-002: Happy Path - Autorizações documentadas por perfil correspondem ao comportamento | documental semântico | passou de imediato; mutação nas permissões faz o teste falhar | **REGRA_JA_IMPLEMENTADA** | nenhum | nenhum | 1 execução aprovada | células CONDICIONAL e PENDENTE não confrontadas; erros por fluxo sem teste próprio |

Casos que continuam sem teste (não são aprovação): T-RF22-001 (viewports), T-RF15-001, T-RF16-001, T-RF17-001, T-RF21-001, T-RF09-001 e T-RF11-003, todos com bloqueio documentado na seção 9.3.

### 11.4 Cobertura por requisito (parcial, não completa)

- **RF-20:** cobertura automatizada PARCIAL (notificação simulada e registro seguro de falha). Não está concluído: eventos, canal real, destinatário e conteúdo.
- **RF-22:** cobertura automatizada PARCIAL (critério 2, parte objetiva). Não está concluído: critério 1 (viewports e interface) segue bloqueado.
- **RF-23:** cobertura automatizada PARCIAL (documentação de estados e permissões confrontada com o comportamento). Não está concluído: artefato oficial, formato e publicação.

### 11.5 T-RN10-003 (situação final)

**GREEN** e sem enfraquecimento: continua exigindo, para cada RF MUST, caso no plano, teste JUnit real (ou caso DUPLICADO cuja cobertura existe em teste real), casos válidos com teste, ausência de identificadores órfãos e classe da matriz igual à do código. Rigor confirmado por mutação: renomear os identificadores dos testes de RF-20 faz T-RN10-003 e T-RN10-005 falharem (RF-20 sem teste, casos válidos sem teste e identificadores órfãos).

### 11.6 Divergências e decisões

- **Reclassificações:** T-RF22-002 (era SEM_COMPORTAMENTO_VERIFICAVEL) e T-RF20-001/002 e T-RF23-001/002 (eram BLOQUEADO_POR_LACUNA). Cada uma cobre só a parte verificável; o que segue indefinido permanece na coluna de lacunas. Nenhum bloqueio dos demais casos foi alterado e os 16 testes @Disabled permanecem.
- **Escolhas de implementação (não são decisões de negócio):** EventoNotificacao tem tipo livre para não fixar os eventos; CanalNotificacao é uma interface para não escolher entre notificação simulada e API externa (J15); a mensagem de falha da notificação é genérica.
- **Documentação como artefato:** os testes de RF-23 tratam docs/fluxos-personas.md como a documentação de fluxos existente. A escolha oficial do artefato segue pendente (FLX-22).
- **WireMock, Testcontainers e dependências:** nada foi adicionado ao pom.xml.

### 11.7 Gate final do QA

| Verificação | Resultado |
|---|---|
| RF-20, RF-22 e RF-23 com ao menos um teste real | Sim (RF-20: 2; RF-22: 5 execuções em 2 métodos; RF-23: 2) |
| Cada teste sustentado pela fonte | Sim (11.2) |
| RED reproduzido antes da implementação | Sim para RF-20 e RF-22 (7 execuções vermelhas por asserção). RF-23 passou de imediato e foi investigado por mutação |
| GREEN pelo comportamento correto | Sim |
| T-RN10-003 sem enfraquecimento | Sim (correção apenas do leitor da matriz; 11.1 e 11.5) |
| Nenhum teste apagado, desabilitado ou enfraquecido; sem exceção artificial | Sim |
| Testes anteriores continuam passando | Sim (os 153 anteriores) |
| Decisão ausente inventada | Não; lacunas restantes documentadas |
| verify final | BUILD SUCCESS, sem skip nem ignore de falha |
| Commit ou push | Nenhum |
